...
public class Auto extends SubsystemBase {
  ...
  public boolean isValidPathSequence(String autoString) {
    ...
    for (int i = 0; i < autoString.length() - 1; i++) {
      ...
      if (Constants.AutoConstants.NOTES.contains(char1)
          && Constants.AutoConstants.NOTES.contains(char2)) {
        setFeedback("Don't go from a note to a note.");
        return false;
      }
      // Checks if there are two scoring spots in a row (not good)
      if (Constants.AutoConstants.SCORING_LOCATIONS.contains(char1)
          && Constants.AutoConstants.SCORING_LOCATIONS.contains(char2)) {
